...
console.log('🔄 جاري إعادة بناء Bundle مع الصور...');
...
const projectFiles = {
  _metadata: {
    ...
    totalImageFiles: 0,
    images: [],
    description: 'Complete project bundle with all files and images',
    lastUpdated: new Date().toISOString(),
  }
};

// جمع جميع ملفات النصوص
  ...
  files.forEach(file => {
    if (['.webp', '.png', '.jpg', '.jpeg', '.gif'].includes(path.extname(file).toLowerCase())) {
      ...
      try {
        ...
        projectFiles[`public/${file}`] = `data:image/${ext};base64,${base64}`;
        projectFiles._metadata.images.push(`public/${file}`);
        projectFiles._metadata.totalImageFiles++;
        console.log(`✅ تم: ${file} (${(buffer.length/1024).toFixed(1)} KB)`);
      } catch (e) {
        console.warn(`❌ خطأ: ${file}`, e.message);
      }
    }
  });
}
...
console.log(`🖼️ ملفات صور: ${projectFiles._metadata.totalImageFiles}`);
